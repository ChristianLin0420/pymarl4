...
class WorldModelingHyperRNNV1Agent(nn.Module):
    def __init__(self, input_shape, args):
        super(WorldModelingHyperRNNV1Agent, self).__init__()
        self.args = args
        ...
        # Recurrent Network
        self.local_rnn = nn.GRUCell(self.args.rnn_hidden_dim, self.args.rnn_hidden_dim)

        # World Modeling (Encoder, Decoder)
        self.wm_latent_size = args.rnn_hidden_dim // args.wm_latent_divider
        self.wm_encoder = nn.Linear(args.rnn_hidden_dim, self.wm_latent_size)
        self.wm_decoder = nn.Linear(self.wm_latent_size, args.rnn_hidden_dim)

        self.global_rnn = nn.GRUCell(self.wm_latent_size, self.wm_latent_size)

        self.fc2_normal_actions = nn.Linear(args.rnn_hidden_dim + self.wm_latent_size, args.output_normal_actions + args.n_enemies)  # (no_op, stop, up, down, right, left)
    ...
    def forward(self, inputs, hidden_state=None):
        ...
        h = self.local_rnn(x, hidden_state)

        # World Modeling
        wm_latent = self.wm_encoder(aggregated_embedding)  # [bs * n_agents, wm_latent_size]
        reconstructed_h = self.wm_decoder(wm_latent) # [bs * n_agents, rnn_hidden_dim]

        # Q-values of normal actions
        q = self.fc2_normal_actions(th.cat((h, wm_latent), dim = -1))  # [bs * n_agents, 6 + n_enemies]

        return q.view(bs, self.n_agents, -1), wm_latent.view(bs, self.n_agents, -1), reconstructed_h.view(bs, self.n_agents, -1), aggregated_embedding.view(bs, self.n_agents, -1) 