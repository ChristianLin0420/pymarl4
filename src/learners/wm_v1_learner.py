...
class WMV1Learner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        obs_mse_loss = self.args.obs_rl_lambda * nn.functional.mse_loss(reconstruction_hidden_state, original_hidden_state) if self.args.use_obs_rl else 0
        state_mse_loss = self.args.state_rl_lambda * nn.functional.mse_loss(reconstruction_state, original_state) if self.args.use_state_rl else 0
        latent_mse_loss = self.args.latent_rl_lambda * nn.functional.mse_loss(wm_latent_obs, wm_state_latent) if self.args.use_latent_rl else 0
